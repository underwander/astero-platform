...
function TradingViewChart({
  ...
  const frameId = compact ? "mobile_tv" : "tradingview_widget";

  return (
    <iframe
      key={`${symbol}-${chartInterval}`}
      src={`https://s.tradingview.com/widgetembed/?frameElementId=${frameId}&symbol=${tvSymbol}&interval=${chartInterval}&hidesidetoolbar=1&symboledit=0&saveimage=0&toolbarbg=10131b&studies=[]&theme=dark&style=1&timezone=Etc%2FUTC&withdateranges=0&hideideas=1&locale=ru`}
      className={`w-full border-0 bg-[#07110f] ${compact ? "h-[310px] touch-pan-y" : "h-[360px] sm:h-[400px] xl:h-[455px] 2xl:h-[500px]"}`}
      allowFullScreen
    />
  );
}
