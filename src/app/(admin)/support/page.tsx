...
export default function SupportPage() {
  ...
  function playNotificationSound() {
    try {
      ...
      oscillator.frequency.setValueAtTime(820, audio.currentTime);
      oscillator.frequency.setValueAtTime(1120, audio.currentTime + 0.12);
  ...
  function pushToast(nextMessage: SupportMessage) {
    setToasts((prev) => {
      const existing = prev.find((item) => item.id === "support");
      if (existing) {
        return prev.map((item) =>
          item.id === "support"
            ? { ...item, message: nextMessage.message || (isRu ? "Файл в поддержке" : "Support file"), count: item.count + 1 }
            : item
        );
      }
      return [{ id: "support", message: nextMessage.message || (isRu ? "Файл в поддержке" : "Support file"), count: 1 }];
    });
    playNotificationSound();
  }

  async function markRead(currentUserId: string) {
    if (!currentUserId) return;
    await fetch("/api/user/support", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      cache: "no-store",
      body: JSON.stringify({ userId: currentUserId, action: "read" }),
    }).catch(() => null);
  }
  ...
  async function loadMessages(currentUserId: string) {
    ...
    if (!res.ok) {
      setToasts([{ id: "error", message: data.error || (isRu ? "Не удалось загрузить поддержку" : "Could not load support"), count: 1 }]);
      return;
    }
    ...
    const newIncomingMessages = nextMessages.filter((item) => {
      const sender = item.sender || item.fromRole;
      return ["ADMIN", "MANAGER", "BOT"].includes(sender || "") && !seenMessageIdsRef.current.has(item.id);
    });

    setConversationStatus(payload.status || "OPEN");
    setMessages(nextMessages);
    setUnreadCount(chatOpenRef.current ? 0 : Number(payload.unreadCount || 0));

    if (readyRef.current && newIncomingMessages.length > 0 && !chatOpenRef.current) {
      newIncomingMessages.forEach(pushToast);
    }

    if (chatOpenRef.current && Number(payload.unreadCount || 0) > 0) {
  ...
  async function sendMessage() {
    ...
    if (!message.trim() && !attachment) {
      setToasts([{ id: "empty", message: isRu ? "Введите сообщение или прикрепите файл" : "Enter a message or attach a file", count: 1 }]);
      return;
    }
    ...
    if (!res.ok) {
      setToasts([{ id: "send-error", message: data.error || (isRu ? "Сообщение не отправлено" : "Message was not sent"), count: 1 }]);
      return;
    }
  ...
  async function attachImage(file: File | null) {
    ...
    if (file.size > 5 * 1024 * 1024) {
      setToasts([{ id: "file-size", message: isRu ? "Файл должен быть до 5MB" : "File must be up to 5MB", count: 1 }]);
      return;
    }
    ...
    setAttachment({ name: file.name, mimeType: file.type || "application/octet-stream", base64 });
  }
  ...
  function openChat() {
    ...
    setUnreadCount(0);
    setToasts([]);
    if (userId) void markRead(userId);
  }
  ...
  useEffect(() => {
    ...
    if (storedRole === "ADMIN" || storedRole === "MANAGER") {
      setToasts([{ id: "role", message: isRu ? "Выйдите из CRM и зайдите как клиент." : "Log out of CRM and sign in as a client.", count: 1 }]);
      return;
    }

    void loadMessages(storedUserId);

    const interval = setInterval(() => void loadMessages(storedUserId), 2500);
  ...
  return (
    <div className="contents">
      {toasts.length > 0 && (
        <div className="fixed right-4 top-24 z-[9998] flex w-[min(340px,calc(100vw-2rem))] flex-col gap-2">
          {toasts.map((toast) => (
            <div key={toast.id} className="rounded-3xl border border-emerald-300/30 bg-slate-950 p-4 text-sm font-black text-white shadow-2xl">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-xs uppercase tracking-[0.18em] text-emerald-300">{isRu ? "Поддержка" : "Support"}</p>
                  <p className="mt-1 line-clamp-3 text-sm text-white">{toast.message}</p>
                  {toast.count > 1 && <p className="mt-1 text-xs text-emerald-200">{isRu ? "Новых сообщений" : "New messages"}: {toast.count}</p>}
                </div>
                <button onClick={() => setToasts((prev) => prev.filter((item) => item.id !== toast.id))} className="rounded-lg bg-white/10 px-2 py-1 text-xs">x</button>
              </div>
              <button onClick={openChat} className="mt-3 w-full rounded-2xl bg-emerald-500 px-3 py-2 text-xs font-black text-slate-950">
      ...
      <button
        ...
        className={`fixed bottom-5 right-5 z-[9998] rounded-full border border-white/30 px-3 py-2 text-[11px] font-black text-slate-950 shadow-xl backdrop-blur-2xl transition hover:border-white/50 hover:bg-emerald-500/35 dark:text-white ${
          unreadCount > 0
            ? "animate-pulse bg-emerald-500/35 shadow-red-500/25 ring-4 ring-red-500/20"
            : "bg-white/12 shadow-emerald-950/10"
        }`}
      >
        {unreadCount > 0 && (
          <span className="absolute -right-1 -top-2 flex size-6 animate-ping items-center justify-center rounded-full bg-red-500/50" />
        )}
        {unreadCount > 0 && (
          <span className="absolute -right-1 -top-2 flex size-6 items-center justify-center rounded-full border-2 border-white bg-red-500 text-[11px] font-black text-white shadow-lg">
        ...
        <div className="fixed left-1/2 top-1/2 z-[9999] max-h-[calc(100dvh-32px)] w-[min(400px,calc(100vw-32px))] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-2xl shadow-slate-950/20 ring-1 ring-black/5 dark:border-white/10 dark:bg-slate-950 sm:bottom-24 sm:left-auto sm:right-6 sm:top-auto sm:translate-x-0 sm:translate-y-0">
          <div className="flex items-center justify-between bg-gradient-to-br from-[#0b2b1d] via-[#0f8a4b] to-[#45d478] px-4 py-4 text-white">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs font-semibold text-white/75">{isRu ? "Чат с менеджером" : "Manager chat"}</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setChatOpen(false)} className="flex size-8 items-center justify-center rounded-full bg-white/16 text-lg font-black transition hover:bg-white/25">x</button>
            </div>
          </div>
              ...
              return (
                <div
                  ...
                  className={`max-w-[86%] px-4 py-3 text-sm shadow-sm md:max-w-[72%] ${
                    isClient
                      ...
                      : isBot
                        ? "mr-auto rounded-[20px] rounded-bl-md border border-sky-100 bg-sky-50 text-sky-950"
                ...
                >
                  <p className="font-semibold">
                    {isClient ? (isRu ? "Вы" : "You") : isBot ? (isRu ? "Бот" : "Bot") : (isRu ? "Менеджер" : "Manager")}
                  </p>
                  ...
                  <p className="mt-2 text-[10px] opacity-60">
                    {new Date(item.createdAt).toLocaleString()} · {isClient ? (isRu ? "доставлено" : "delivered") : (isRu ? "получено" : "received")}
                  </p>
                </div>
              );
            })}
            {isSending && <p className="text-xs font-bold text-slate-400">{isRu ? "Отправляем..." : "Sending..."}</p>}
          ...
          <div className="border-t border-slate-200 bg-white p-3 dark:border-white/10 dark:bg-slate-950">
            <div className="flex items-end gap-2 rounded-2xl border border-slate-200 bg-slate-50 p-2 dark:border-white/10 dark:bg-white/[0.04]">
              ...
              />
              <button onClick={() => void sendMessage()} className="h-10 shrink-0 rounded-full bg-emerald-600 px-5 text-sm font-black text-white shadow-md shadow-emerald-900/15 transition hover:bg-emerald-500">
                {isRu ? "Отправить" : "Send"}
              </button>
            </div>
            {attachment && (
              <button type="button" onClick={() => setAttachment(null)} className="mt-2 rounded-lg bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-700">
                {isRu ? "Прикреплено: " : "Attached: "}{attachment.name} x
              </button>
            )}
          </div>
        </div>
