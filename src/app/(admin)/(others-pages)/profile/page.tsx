...
export default function ProfilePage() {
  ...
  async function saveProfile() {
    ...
    const data = await res.json();
    setMessage(res.ok ? (language === "ru" ? "Профиль сохранён" : "Profile saved") : data.error || "Error");
  ...
  async function uploadDocument() {
    ...
    const data = await res.json();
    setUploadMessage(res.ok ? (language === "ru" ? "Документ отправлен на проверку" : "Document submitted for review") : data.error || "Upload error");
    if (res.ok) {
      setFile(null);
      await loadProfile();
    }
  ...
  async function changePassword() {
    ...
    const res = await fetch("/api/user/change-password", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ userId: profile.id, currentPassword, newPassword }),
    });

    const data = await res.json();
    setPasswordMessage(res.ok ? (language === "ru" ? "Пароль изменён" : "Password changed") : data.error || "Password error");
    if (res.ok) {
      setCurrentPassword("");
      setNewPassword("");
    }
  ...
  if (!profile) {
    return <div className="rounded-[2rem] border border-emerald-100 bg-white p-6 dark:border-emerald-400/10 dark:bg-white/[0.04]">Loading...</div>;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="overflow-hidden rounded-[2rem] border border-emerald-400/20 bg-gradient-to-br from-[#06130d] via-[#092016] to-emerald-950 p-5 text-white shadow-xl shadow-emerald-950/10 md:p-8">
        ...
        <h1 className="mt-4 text-3xl font-black tracking-tight md:text-4xl">{t("profile")}</h1>
        <p className="mt-2 max-w-2xl text-sm leading-6 text-emerald-50/70">
          {language === "ru" ? "Личные данные, верификация документов и смена пароля." : "Personal data, document verification and password management."}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
        <div className="rounded-[2rem] border border-emerald-100 bg-white p-5 shadow-sm dark:border-emerald-400/10 dark:bg-white/[0.04] xl:col-span-2">
          <h2 className="text-lg font-black text-slate-900 dark:text-white">{t("personalInfo")}</h2>
        ...
        <div className="rounded-[2rem] border border-emerald-100 bg-white p-5 shadow-sm dark:border-emerald-400/10 dark:bg-white/[0.04]">
          <h2 className="text-lg font-black text-slate-900 dark:text-white">KYC</h2>
          <p className="mt-2 text-sm text-slate-500 dark:text-emerald-50/60">Status: <b>{profile.kycStatus}</b></p>
          <div className="mt-5 space-y-3">
            ...
            {uploadMessage && <p className="text-sm text-emerald-600 dark:text-emerald-300">{uploadMessage}</p>}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
        <div className="rounded-[2rem] border border-emerald-100 bg-white p-5 shadow-sm dark:border-emerald-400/10 dark:bg-white/[0.04]">
          <h2 className="text-lg font-black text-slate-900 dark:text-white">{t("security")}</h2>
          <div className="mt-5 space-y-3">
            ...
            {passwordMessage && <p className="text-sm text-emerald-600 dark:text-emerald-300">{passwordMessage}</p>}
          </div>
        </div>

        <div className="rounded-[2rem] border border-emerald-100 bg-white p-5 shadow-sm dark:border-emerald-400/10 dark:bg-white/[0.04]">
          <h2 className="text-lg font-black text-slate-900 dark:text-white">{t("verification")}</h2>
          <div className="mt-4 space-y-3">
            {profile.verificationDocuments.length === 0 && <p className="text-sm text-slate-500">No documents uploaded</p>}
            {profile.verificationDocuments.map((doc) => (
              <div key={doc.id} className="rounded-2xl border border-emerald-100 p-4 text-sm dark:border-emerald-400/10">
                <p className="font-bold text-slate-900 dark:text-white">{doc.documentType}</p>
                <p className="mt-1 text-slate-500">{doc.fileName}</p>
                <p className="mt-1 font-bold text-emerald-600">{doc.status}</p>
              </div>
            ))}
          </div>
        </div>
