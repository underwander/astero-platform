...
function emptyForm() {
  ...
  return { clientId: "", type: "TASK", title: "", description: "", dueAt: localInput(start), endAt: "", allDay: false, priority: "NORMAL", reminderMinutes: "15", managerId: "" };
}
  ...
  return (
    <div className="actions-workspace space-y-4">
      <header className="flex flex-wrap items-end justify-between gap-4">
        ...
        <button onClick={openCreate} className="rounded-lg bg-emerald-700 px-4 py-2.5 text-sm font-semibold text-white hover:bg-emerald-800">+ Назначить задачу</button>
      </header>
...
function InlineActionRow({ action, form, setForm, managers, busy, onSave, onCancel }: { action: ActionItem; form: ActionForm; setForm: React.Dispatch<React.SetStateAction<ActionForm>>; managers: Person[]; busy: boolean; onSave: () => Promise<void>; onCancel: () => void }) {
  ...
  return <tr className="border-y border-emerald-200 bg-emerald-50/40 align-top" onKeyDown={handleKeyDown}>
    ...
    <td className="min-w-32 px-2 py-2"><span aria-label="Тип действия" className="inline-flex rounded-full bg-slate-100 px-2 py-1 text-xs font-semibold">{typeNames[form.type] || "Задача"}</span></td>
    ...
    <td className="min-w-64 space-y-1 px-2 py-2">
      <EmojiTextField aria-label="Название действия" required value={form.title} onChange={(value) => setForm((current) => ({ ...current, title: value }))} className={`${inputClass} w-full font-semibold`} />
      <EmojiTextField aria-label="Описание действия" multiline value={form.description} onChange={(value) => setForm((current) => ({ ...current, description: value }))} rows={2} className={`${inputClass} max-h-24 min-h-14 w-full resize-y`} />
    </td>
...
function CreateActionModal({ form, setForm, clients, managers, busy, close, submit }: { form: ActionForm; setForm: React.Dispatch<React.SetStateAction<ActionForm>>; clients: Client[]; managers: Person[]; busy: boolean; close: () => void; submit: (event: React.FormEvent) => void }) {
  return <Modal title="Назначить задачу" close={close}><form onSubmit={submit} className="grid gap-3 sm:grid-cols-2"><Field label="Клиент *"><select required value={form.clientId} onChange={(event) => setForm({ ...form, clientId: event.target.value })}><option value="">Выберите клиента</option>{clients.map((client) => <option key={client.id} value={client.id}>{personName(client)} · {client.email}</option>)}</select></Field><Field label="Название *" wide><EmojiTextField required value={form.title} onChange={(value) => setForm({ ...form, title: value })} /></Field><Field label="Описание" wide><EmojiTextField multiline value={form.description} onChange={(value) => setForm({ ...form, description: value })} /></Field><Field label="Срок *"><input required type="datetime-local" value={form.dueAt} onChange={(event) => setForm({ ...form, dueAt: event.target.value })} /></Field><Field label="Ответственный"><select value={form.managerId} onChange={(event) => setForm({ ...form, managerId: event.target.value })}><option value="">Не назначен</option>{managers.map((item) => <option key={item.id} value={item.id}>{personName(item)}</option>)}</select></Field><Field label="Приоритет"><select value={form.priority} onChange={(event) => setForm({ ...form, priority: event.target.value })}>{Object.entries(priorityNames).map(([key,label]) => <option key={key} value={key}>{label}</option>)}</select></Field><Field label="Напоминание"><select value={form.reminderMinutes} onChange={(event) => setForm({ ...form, reminderMinutes: event.target.value })}><option value="">Нет</option>{[0,5,15,30,60,120,1440].map((value) => <option key={value} value={value}>{value === 0 ? "В момент задачи" : value < 60 ? `За ${value} минут` : `За ${value / 60} ч.`}</option>)}</select></Field><div className="col-span-full flex justify-end gap-2 pt-2"><button type="button" onClick={close} className="rounded-lg border px-4 py-2">Отмена</button><button disabled={busy} className="rounded-lg bg-emerald-700 px-4 py-2 font-semibold text-white">Сохранить</button></div></form></Modal>;
}
...
function CompleteModal({ action, managers, busy, close, save }: { action: ActionItem; managers: Person[]; busy: boolean; close: () => void; save: (payload: object) => Promise<void> }) {
  ...
  return <Modal title="Завершить задачу" close={close}><div className="space-y-3"><Field label="Результат *"><select value={outcome} onChange={(event) => setOutcome(event.target.value)}>{["SUCCESSFUL","NO_ANSWER","CALL_BACK","INTERESTED","NOT_INTERESTED","KYC_REQUESTED","KYC_COMPLETED","DEPOSIT_EXPECTED","CONVERTED","OTHER"].map((item) => <option key={item}>{item.replaceAll("_", " ")}</option>)}</select></Field><Field label="Комментарий"><EmojiTextField multiline value={note} onChange={setNote} /></Field><label className="flex items-center gap-2 text-sm font-semibold"><input type="checkbox" checked={next} onChange={(event) => setNext(event.target.checked)} /> Создать следующую задачу</label>{next && <div className="grid gap-3 rounded-xl bg-slate-50 p-4 sm:grid-cols-2"><Field label="Название"><EmojiTextField value={title} onChange={setTitle} /></Field><Field label="Дата и время"><input type="datetime-local" value={due} onChange={(event) => setDue(event.target.value)} /></Field><Field label="Ответственный"><select value={manager} onChange={(event) => setManager(event.target.value)}><option value="">Не назначен</option>{managers.map((item) => <option key={item.id} value={item.id}>{personName(item)}</option>)}</select></Field></div>}<div className="flex justify-end gap-2"><button onClick={close} className="rounded-lg border px-4 py-2">Отмена</button><button disabled={busy} onClick={() => { const nextDue = validDate(due); void save({ status: "CLOSED", outcome, outcomeNote: note, nextAction: next && nextDue ? { title, dueAt: nextDue.toISOString(), managerId: manager, type: "TASK", reminderMinutes: 15 } : null }); }} className="rounded-lg bg-emerald-700 px-4 py-2 font-semibold text-white">Завершить</button></div></div></Modal>;
}
